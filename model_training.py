...
def train_model(model_class, model_params, exp_params, scoring_funcs,
                use_mlflow=False, eval_testing=False, save_model=True, save_model_path=None):
    """
    Args:
    ...
    X_train, X_val, y_train, y_val = train_test_split(
        X_train, y_train, test_size=val_size, random_state=exp_params.get('train_test_split_seed', None))
    ...
    scores.update(evaluate_builtin_metric(model, X_train, y_train,
                                          scoring_funcs, prob_threshold=prob_threshold, index_predix='train-'))
    scores.update(evaluate_builtin_metric(model, X_val, y_val,
                                          scoring_funcs, prob_threshold=prob_threshold, index_predix='val-'))

    eval_roc_auc_display(model, X_train, y_train,
                         fig_path='train-auc.png', use_mlflow=use_mlflow)
    eval_roc_auc_display(model, X_val, y_val,
                         fig_path='val-auc.png', use_mlflow=use_mlflow)
    ...
    if eval_testing:
        ...
        scores.update(evaluate_builtin_metric(model_test, X_test, y_test,
                                              scoring_funcs, prob_threshold=prob_threshold, index_predix='test-'))
        eval_roc_auc_display(model_test, X_test, y_test,
                             fig_path='test-auc.png', use_mlflow=use_mlflow)
...
def train_cv_model(model, model_params, exp_params, scoring,
